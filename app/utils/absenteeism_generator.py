...
def generate_absenteeism_data():
    ...
    data = {
        'Colaborador_id': [
            faker.unique.random_number(digits=5) for _ in range(10)
        ],
        'Colaborador_nome': [faker.name() for _ in range(10)],
        'Departamento': [
            faker.random_element(elements=departments) for _ in range(10)
        ],
        'Motivo_da_ausência': [
            faker.random_element(elements=reasons) for _ in range(10)
        ],
        'Horas_de_ausência': [
            faker.random_int(min=1, max=8) for _ in range(10)
        ],
        'Data_da_ausência': [
